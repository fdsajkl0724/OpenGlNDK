...
void SkyBoxSample::Init()
{
	...
	{
		LOGCATE("SkyBoxSampleTest::Init create program fail");
		return;
	}
...
{
	LOGCATE("SkyBoxSampleTest::LoadImage pImage = %p", pImage->ppPlane[0]);
	if (pImage)
	{
...
{
	LOGCATE("SkyBoxSampleTest::LoadMultiImageWithIndex pImage = %p, index=%d", pImage->ppPlane[0], index);
	if (pImage)
	{
...
{
	LOGCATE("SkyBoxSampleTest::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX,
			angleY, ratio);
	angleX = angleX % 360;
	angleY = angleY % 360;
