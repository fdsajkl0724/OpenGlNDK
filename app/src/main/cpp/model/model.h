...
public:
    ...
    // draws the model, and thus all its meshes 渲染模型，即依次渲染各个网格
...
private:
    /*  Functions   */
    // loads a model with supported ASSIMP extensions from file and stores the resulting meshes in the meshes vector.//加载模型
    ...
    unsigned int TextureFromFile(const char *path, const string &directory, bool gamma = false)
    {
        ...
        // load the texture using OpenCV    //使用 OpenCV 对图片进行解码
        LOGCATE("TextureFromFile Loading texture %s", filename.c_str());
        cv::Mat textureImage = cv::imread(filename);
        if (!textureImage.empty())
        {
            hasTexture = true;
            // opencv reads textures in BGR format, change to RGB for GL  OpenCV 默认解码成 BGR 格式，这里转换为 RGB
